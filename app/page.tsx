        ...
        {isClient && !isLoading && isTrialEndingSoon && !isTrialExpired && (
          <div className="p-6">
            <Alert className="bg-yellow-50 border-yellow-200 text-yellow-800 dark:bg-yellow-900/20 dark:border-yellow-800 dark:text-yellow-200">
              ...
              <AlertDescription>
                無料試用期間は残り <span className="font-bold">{daysRemaining}日</span> です。サービスを継続してご利用いただくには、お早めに購読プランをご検討ください。
                <Button 
                  variant="link" 
                  onClick={() => setShowPricing(true)}
                  className="p-0 h-auto text-yellow-800 hover:text-yellow-900 dark:text-yellow-200 dark:hover:text-yellow-100 ml-2"
                >
                  購読プランを見る
                </Button>
              </AlertDescription>
